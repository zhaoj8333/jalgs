...
public class SkipListBak<K, V> {
     ...
     * 首节点不存放任何K-V
     */
    private NodeBak<K, V> first;

    public SkipListBak() {
        this(null);
    }

    public SkipListBak(Comparator<K> comparator) {
        this.comparator = comparator;
        first  = new NodeBak<>(null, null, MAX_LEVEL);
        first.nexts = new NodeBak[MAX_LEVEL];
    }
    ...
    public V put(K key, V val) {
        keyCheck(key);
        NodeBak<K, V> node = first;
        // 每一层的前驱节点
        NodeBak<K, V>[] prevs = new NodeBak[level];
        ...
        NodeBak<K, V> newNode = new NodeBak<>(key, val, newLevel);
    ...
    public V remove(K key) {
        ...
        NodeBak<K, V> removedNode = node.nexts[0];
        // 设置后继
    ...
    private static class NodeBak<K, V> {
        K key;
        V val;
        NodeBak<K, V>[] nexts;

        public NodeBak(K key, V val, int level) {
            this.key = key;
            this.val = val;
            this.nexts = new NodeBak[level];
        }
    ...
    public static void main(String[] args) {
        final SkipListBak<Integer, Integer> list = new SkipListBak<>();
        test(list, 20, 10);
    }

    private static void test(SkipListBak<Integer, Integer> list, int count, int delta) {
