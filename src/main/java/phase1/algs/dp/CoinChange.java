 ...
 *             phase1.algs.dp(i, 0) = phase1.algs.dp(0, j) = 1
 *             phase1.algs.dp(i, j) = phase1.algs.dp(i, j - 1) + phase1.algs.dp(i - 1, j)
 *             推导 phase1.algs.dp(i, j)只需要用到dp(i, j- 1),phase1.algs.dp(i - 1, j)的值,并不需要关心dp(i, j - 1), phase1.algs.dp(i - 1, j)是如何推导出来的
 *     有后效性: 无法用动态规划解决
...
//        coinCount = dp3Bak(faces, 41);
    ...
    private static int dp3(Integer[] faces, int money) {
        ...
        for (int i = 1; i <= money; i++) {
            count++;
//            phase1.algs.dp[i] = min{phase1.algs.dp[i - 25], phase1.algs.dp[i - 20], phase1.algs.dp[i - 5], phase1.algs.dp[i - 1]} + 1;
            int min = dp[i - 1];
            if (i >= 5) {
...
//            phase1.algs.dp[i] = min{phase1.algs.dp[i - 25], phase1.algs.dp[i - 20], phase1.algs.dp[i - 5], phase1.algs.dp[i - 1]} + 1;
    ...
    private static int dp2(int money, int[] dp) {
        ...
        // System.out.println(money + " : " + Arrays.toString(phase1.algs.dp));
        return dp[money];
    }
     ...
     *      phase1.algs.dp(41): 凑到41分的最少硬币个数
     *  如果第一次选择了25分的硬币,则dp(n) = phase1.algs.dp(n - 25) + 1
     *  如果第一次选择了20分的硬币,则dp(n) = phase1.algs.dp(n - 20) + 1
     *  如果第一次选择了5分的硬币,则dp(n)  = phase1.algs.dp(n - 5) + 1
     *  如果第一次选择了1分的硬币,则dp(n)  = phase1.algs.dp(n - 1) + 1
     *
     *  所以, phase1.algs.dp(n) = min {
     *      phase1.algs.dp(n-25))
     *      phase1.algs.dp(n-20))
     *      phase1.algs.dp(n-5))
     *      phase1.algs.dp(n-1))
