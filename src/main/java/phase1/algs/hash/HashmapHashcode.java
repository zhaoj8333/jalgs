...
//        System.out.println(Integer.toBinaryString(i ^ j)); // 4228
     ...
     *
     *     1. 直接寻址法：
     *         phase1.algs.hash(key) = key
     *         取关键字key的某个线性函数值为散列地址，即hash(key) = a x key + b
     *         该方法简单，均匀，不会有冲突，但是必须知道关键字key的分布情况，适合表较小且连续的情况，不常用
     *
     *     2. 除留余数法： 对于列表长为m的散列函数公式为 phase1.algs.hash(key) = key mod p(p <= m)
     *         该方法关键在于选择合适的p，如果p选的不好可能产生hash冲突
     *         比如有12个关键字key，则为 phase1.algs.hash(key) = key mod 12，但是比如有12,18，都会为6，则会产生hash冲突
     *         使用该方法的经验是：
     *              如果散列表为m，通常p为小于或等长（接近m）的最大质数或不包含小于20质因子的合数
     *              总之当p取小于m的最大质数时，产生的哈希值比较好
     *
     *     3. 平方取中法：
     *         phase1.algs.hash(key) = key 平方的中间几位
     ...
     *     key1 != key2, 但是 phase1.algs.hash(key1) == phase1.algs.hash(key2)
...
//        System.out.println(Integer.toBinaryString(phase1.algs.hash));
