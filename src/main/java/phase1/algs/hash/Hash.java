...
//        System.out.println(phase1.algs.hash(1));
//        intHash();
//        floatHash();
        stringHash();
...
//     使用素数31作为乘数，因为 31 * i == (i << 5) - i, 效率更高
                    System.out.println(Integer.toBinaryString((int)chars[j]));
                }
                hash = h;
            }
//            System.out.println(str[i] + " : " + h);
        }
    }
...
//            System.out.println(num[i].hashCode());
        ...
        for (int i = 0; i < num.length; i++) {
//            System.out.println((int)(num[i] * 10000) % 97f);
//            System.out.println(Float.toHexString(num[i]));
    ...
    private static void intHash() {
        ...
        System.out.println();
        for (int i = 0; i < num.length; i++) {
            System.out.print(num[i] % 97 % num.length + " ");
        }
        System.out.println();
    }
     ...
     *     2. 如果key1 == key2，那么hash(key1) == phase1.algs.hash(key2)
     *     3. 如果key1 == key2，那么hash(key1) != phase1.algs.hash(key2)
     *
     * 好的哈希函数必须：
     * 哈希函数不能太复杂
