 ...
 *             推导 dp(i, j)只需要用到dp(i, j- 1),dp(i - 1, j)的值,并不需要关心dp(i, j - 1), dp(i - 1, j)是如何推导出来的
 *     有后效性: 无法用动态规划解决
...
//            dp[i] = min{dp[i - 25], dp[i - 20], dp[i - 5], dp[i - 1]} + 1;
    ...
    private static int dp2(int money, int[] dp) {
        ...
        // StdOut.println(money + " : " + Arrays.toString(dp));
        return dp[money];
    }
     ...
     *      dp(41): 凑到41分的最少硬币个数
     *  如果第一次选择了25分的硬币,则dp(n) = dp(n - 25) + 1
     *  如果第一次选择了20分的硬币,则dp(n) = dp(n - 20) + 1
     *  如果第一次选择了5分的硬币,则dp(n)  = dp(n - 5) + 1
     *  如果第一次选择了1分的硬币,则dp(n)  = dp(n - 1) + 1
     *
     *  所以, dp(n) = min {
     *      dp(n-25))
     *      dp(n-20))
